...
.config-list-item-actions .delete-btn:hover {
    background-color: #fee2e2;
    color: var(--danger-color);
}
body.dark .config-list-item-actions .delete-btn:hover {
...
.data-source-selector-container .form-label {
    margin-bottom: 0;
    flex-shrink: 0;
}
.data-source-selector-container .form-label i {
    margin-right: 0.5rem;
    color: var(--primary-color);
}
.data-source-selector-container .input-style {
    max-width: 300px;
}

/* --- Read Only Mode --- */
.read-only-mode form,
.read-only-mode .main-actions-group,
.read-only-mode .accessory-adder,
.read-only-mode #add-custom-component-btn {
    opacity: 0.6;
    pointer-events: none;
}
.read-only-mode .table-wrapper {
    pointer-events: auto; /* Allow scrolling */
}
.read-only-mode .component-table input,
.read-only-mode .component-table .combobox-input {
    background-color: var(--app-bg);
}
.read-only-mode .remove-component-btn {
    visibility: hidden;
}
.read-only-mode td[data-label="Thao tác"] button:not(.view-btn) {
    display: none;
}
